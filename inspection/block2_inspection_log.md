# Lab 5a Meeting Inspection Issue Log: Block 2
*The recorder for this block should fill out this form.*

## Inspection Roles
*Rotate roles for each code block, so that each participant has a chance to play each role.*
| Author | Moderator | Recorder | Reader | Inspector |
|--------|-----------|----------|--------|-----------|
| Chris| Savannah   | Adrien   | Alex  | Kyle      | 

## Inspection Summary
|  |  |
|-------|-------|
| **Code Block #**: | [2] |
| **Meeting Date:** | [4/29/2026] |
| **Time Started:** | [8:30pm] |
| **Time Finished:** | [8:40pm] |
| **Minutes Spent:** | [10min] |
| **Defects Found:** | [0] major, [4] minor|

## Defects

### Legend
| Column      | Options |
|-------------|---------|
| Type        | Structure, Documentation, Variables, Arithmetic Operations, Branches and Loops, Defensive Programming, Other |
| Severity    | Major, Minor |
| Description | Brief description of the defect |

### Log

| # | Line # | Type | Severity | Description   |
|---|--------|------|----------|---------------|
| 1 |    3    |   structure   |     minor     |       head block should be indented        |
| 2 |    6    |   structure   |    minor      |       body block should be indented        |
| 3 |    16    |   documentation   |     minor     |      add item function has no documentation         |
| 4 |    19    |   defensive programming   |     minor     |       better input validatin        |

